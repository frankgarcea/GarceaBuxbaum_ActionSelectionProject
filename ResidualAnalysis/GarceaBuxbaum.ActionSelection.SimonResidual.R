...
# load data of high and low conflict RT from the Simon task.
data <- read_excel("ConflictResProject.SimonData.xlsx")
...
data$Simon_Incongruent_Resid <- residuals(model)
...
### SAVE
write.csv(data,'ConflictResProject.Simon.IncongruentResiduals.LogTransformed.csv')
